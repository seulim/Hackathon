...
function ImageRotation() {
	...
	function getSize(){
		var li = tmpListId.getElementsByTagName('li')[0];
		var w = li.offsetWidth + parseInt(li.style.marginLeft) + parseInt(li.style.marginRight);
		if(!(w > 0)){
			w = li.offsetWidth;
		}
		objWidth = w * tmpNum;
		if(!widthNoChange){
			tmpListId.style.width = (objNum * w*2) + 'px';
		}
	}
    ...
    this.initialize = function () {
		...
		widthNoChange = this.widthNoChange == "yes" ? true : false;
		cntRoll = this.sRoll;
...
//function ImageRotation() {
//    var scroll = {time:1, start:0, change:0, duration:5, timer:null};
//    var originaltime = scroll.time;
//    var objWidth = 0;
//    var currentNum = 0;
...
//
//
//		liWidth = tmpListId.getElementsByTagName('li')[0].offsetWidth + totMg; // + parseInt(li.style.marginLeft) + parseInt(li.style.marginRight);
//
...
$.fn.moreCategory2 = function(moreBtn, defaultCount){
	...
	var btn = self.find(moreBtn);
	var isBtn = (btn == undefined) ? false : true;
	var obj_close = self.find(".btn_close");

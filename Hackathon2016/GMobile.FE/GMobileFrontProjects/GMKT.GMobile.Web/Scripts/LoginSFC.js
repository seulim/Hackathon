$(function () {
	if (typeof mobileLocalStorage !== "undefined" && mobileLocalStorage !== null) {
		var id = mobileLocalStorage.getItem("GMKTLoginSFCID");
...
function checkValid() {
	if ((document.loginform.id.value == "") || trim(document.loginform.id.value) == "") {
		alert("아이디를 입력해 주세요.");
		document.loginform.id.focus();
		return;
	}

	if (getByteLength(document.loginform.id.value) > 10) {
		alert("아이디 확인 후 다시 입력해 주세요.");
		document.loginform.id.focus();
		return;
	}

	if ((document.loginform.pwd.value == "") || trim(document.loginform.pwd.value) == "") {
		alert("비밀번호를 입력해 주세요.");
		document.loginform.pwd.focus();
		return;
	}

	if (getByteLength(document.loginform.pwd.value) > 15) {
	...
	if (typeof mobileLocalStorage !== "undefined" && mobileLocalStorage !== null) {
		if ($("#save_id").is(":checked")) {
			mobileLocalStorage.setItem("GMKTLoginSFCID", $("#id").val());
		} else {
			mobileLocalStorage.removeItem("GMKTLoginSFCID");
		}
	}
